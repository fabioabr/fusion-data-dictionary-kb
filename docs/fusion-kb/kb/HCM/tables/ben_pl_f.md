...
id: DOC-HCM-055
doc_type: system-doc
title: "BEN_PL_F — Planos de Benefício"
...
Armazena os **planos de benefício** disponíveis (ex.: plano de saúde, plano dental, seguro de vida, previdência). Cada plano define o tipo de benefício, regras, taxas e opções disponíveis.
...
- **Definição de benefícios:** Tabela central de planos de benefício.
- **Configuração:** Regras, taxas e opções por plano.
- **Elegibilidade:** Base para avaliação de elegibilidade.
- **Integração com folha:** Planos vinculados a deduções.
...
### Consulta de planos de benefício
```sql
SELECT * FROM BEN_PL_F
WHERE  ROWNUM <= 100;
```
...
- Tabela do módulo Benefits (Planos de Benefício).

---

## 📚 Referências

- [Oracle Docs — BEN_PL_F](https://docs.oracle.com/en/cloud/saas/human-resources/25a/oedmf/benplf.html)
