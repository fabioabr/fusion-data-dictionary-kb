...
id: DOC-HCM-183
doc_type: system-doc
title: "HRQ_CATEGORIES_TL — Traduções de Categorias de Questionários"
...
aliases:
  - HRQ_CATEGORIES_TL
  - hrq_categories_tl
  - traduções-de-categorias-de-questionários
...
Tabela de **traduções** de categorias de questionários (base). Padrão Oracle MLS.
...
|---|--------|------|-------|-----------|-----------|-----|-----------|
| 1 | CATEGORY_ID | NUMBER(18) | NOT NULL | PK/FK | Identificador | [[hrq_categories_b]] | 🟢 95% |
...
- [[hrq_categories_b]] — via `CATEGORY_ID` (registro base do cadastro)
...
FROM   HRQ_CATEGORIES_TL tl
WHERE  tl.CATEGORY_ID = :p_id AND tl.LANGUAGE = USERENV('LANG');
```
...
- Chave primária composta: `CATEGORY_ID` + `LANGUAGE`.
