...
id: DOC-HCM-638
doc_type: system-doc
title: "PER_CALENDAR_EVENTS_TL — Eventos de Calendário (Traduções)"
...
aliases:
  ...
  - per_calendar_events_tl
  - per-calendar-events-tl
  - eventos-de-calendário-(traduções)
  - per-calendar-events-
...
Armazena as **traduções** dos nomes dos eventos de calendário em múltiplos idiomas.
...
- **Internacionalização** — exibe nomes de feriados e eventos no idioma do usuário.
- **Relatórios localizados** — suporte multilíngue para calendários.
- **Consistência** — tradução centralizada de nomes de eventos.
---
...
|---|--------|------|-------|-----------|-----------|-----|-----------|
| 1 | CALENDAR_EVENT_ID | NUMBER(18) | NOT NULL | PK/FK | Identificador do evento | PER_CALENDAR_EVENTS | 🟢 95% |
...
| 4 | EVENT_NAME | VARCHAR2(240) | NOT NULL | Tradução | Nome traduzido do evento | — | 🟢 90% |
...
```sql
SELECT tl.CALENDAR_EVENT_ID, tl.EVENT_NAME
FROM   PER_CALENDAR_EVENTS_TL tl
