---
id: DOC-AR-032
doc_type: system-doc
title: "AR_AGING_BUCKET_LINES_B — Linhas de Faixas de Aging"
system: Oracle Fusion Cloud ERP
module: Accounts Receivable
domain: Técnico
owner: fabio.patria
team: dados
status: draft
confidentiality: internal
tags: [oracle-fusion, accounts-receivable, data-dictionary, aging, bucket-lines, faixas]
aliases: [AR_AGING_BUCKET_LINES_B, ar_aging_bucket_lines_b, aging_bucket_lines, linhas_aging, bucket_lines_b]
source_format: markdown
conversion_pipeline: manual-v1
conversion_quality: 100
qa_score: 0
qa_date: 2026-03-25
qa_status: not_reviewed
created_at: 2026-03-25
updated_at: 2026-03-25
---

# AR_AGING_BUCKET_LINES_B

## 📌 Visão Geral

Tabela base (idioma neutro) que armazena as linhas individuais de cada faixa de aging. Cada linha define um intervalo de dias (ex.: 0–30, 31–60) dentro de um bucket de aging. A tabela _B contém os dados estruturais; as traduções ficam em [[ar_aging_bucket_lines_tl]].

## 🧠 Propósito de Negócio

Cada faixa de aging é composta por múltiplas linhas que definem os intervalos de dias de atraso. Essas linhas determinam como os saldos em aberto são distribuídos nos relatórios de aging, permitindo análise granular da inadimplência por período. A configuração correta das linhas é essencial para relatórios de cobrança e provisão de perdas.

## ⚙️ Colunas Principais

> [!tip] Confiança
> Escala de 0% a 100% — grau de certeza da descrição gerada por IA com base na documentação oficial Oracle (OEDMF/BICC Release 13/25A).
> - 🟢 **81–100%** — Coluna presente na documentação oficial Oracle; nome, tipo e descrição confirmados.
> - 🟡 **51–80%** — Coluna inferida por naming convention ou padrão Oracle; tipo exato pode variar.
> - 🔴 **0–50%** — Existência ou tipo incertos; pode não existir no release atual; validar no ambiente.

| # | Coluna | Tipo | Nulo? | Descrição | Categoria | Confiança |
|---|--------|------|-------|-----------|-----------|-----------|
| 1 | AGING_BUCKET_LINE_ID | NUMBER(15) | NOT NULL | Chave primária. Identificador único da linha de aging. | PK | 🟢 100% |
| 2 | AGING_BUCKET_ID | NUMBER(15) | NOT NULL | FK para [[ar_aging_buckets]]. Bucket ao qual a linha pertence. | FK | 🟢 100% |
| 3 | BUCKET_SEQUENCE_NUM | NUMBER(15) | NOT NULL | Número de sequência da linha dentro do bucket. | Negócio | 🟢 100% |
| 4 | DAYS_START | NUMBER | NULL | Dia inicial do intervalo de aging. | Negócio | 🟢 100% |
| 5 | DAYS_TO | NUMBER | NULL | Dia final do intervalo de aging. | Negócio | 🟢 100% |
| 6 | TYPE | VARCHAR2(30) | NOT NULL | Tipo da linha (ex.: DAYS_RANGE, AMOUNT_RANGE). | Classificação | 🟢 100% |
| 7 | REPORT_HEADING1 | VARCHAR2(15) | NULL | Cabeçalho para exibição em relatórios (linha 1). | Apresentação | 🟢 100% |
| 8 | REPORT_HEADING2 | VARCHAR2(15) | NULL | Cabeçalho para exibição em relatórios (linha 2). | Apresentação | 🟢 100% |
| 9 | ATTRIBUTE_CATEGORY | VARCHAR2(30) | NULL | Contexto de flexfield descritivo. | DFF | 🟢 100% |
| 10 | ATTRIBUTE1 | VARCHAR2(150) | NULL | Atributo descritivo flexível 1. | DFF | 🟢 100% |
| 11 | ATTRIBUTE2 | VARCHAR2(150) | NULL | Atributo descritivo flexível 2. | DFF | 🟢 100% |
| 12 | CREATED_BY | VARCHAR2(64) | NOT NULL | Usuário que criou o registro. | WHO | 🟢 100% |
| 13 | CREATION_DATE | DATE | NOT NULL | Data de criação do registro. | WHO | 🟢 100% |
| 14 | LAST_UPDATED_BY | VARCHAR2(64) | NOT NULL | Usuário da última atualização. | WHO | 🟢 100% |
| 15 | LAST_UPDATE_DATE | DATE | NOT NULL | Data da última atualização. | WHO | 🟢 100% |

## 🔗 Relacionamentos

- **[[ar_aging_buckets]]** — Bucket pai (N:1 via `AGING_BUCKET_ID`).
- **[[ar_aging_bucket_lines_tl]]** — Traduções desta linha (1:N via `AGING_BUCKET_LINE_ID`).

## 📎 Uso Típico

```sql
-- Detalhe das faixas de aging para um bucket específico
SELECT bl.BUCKET_SEQUENCE_NUM,
       bl.DAYS_START,
       bl.DAYS_TO,
       bl.TYPE,
       bl.REPORT_HEADING1
  FROM AR_AGING_BUCKET_LINES_B bl
 WHERE bl.AGING_BUCKET_ID = :p_bucket_id
 ORDER BY bl.BUCKET_SEQUENCE_NUM;
```

## 🔒 Observações

- A coluna `BUCKET_SEQUENCE_NUM` define a ordem de exibição nos relatórios de aging.
- O par (`AGING_BUCKET_ID`, `BUCKET_SEQUENCE_NUM`) deve ser único.
- Intervalos de dias (`DAYS_START`, `DAYS_TO`) não devem se sobrepor dentro do mesmo bucket.

## 🔗 PVOs Relacionados

### [[agingbucketlinesbasepvo|AgingBucketLinesBasePVO]] (OTHER · BICC: 8/15)

| Coluna da Tabela | Atributo do PVO | BICC |
|------------------|-----------------|------|
| AGING_BUCKET_ID | AgingBucketsLinesAgingBucketId | — |
| AGING_BUCKET_LINE_ID | AgingBucketLineId | ✅ |
| BUCKET_SEQUENCE_NUM | AgingBucketsLinesBucketSequenceNum | ✅ |
| CREATED_BY | AgingBucketsLinesCreatedBy | ✅ |
| CREATION_DATE | AgingBucketsLinesCreationDate | ✅ |
| DAYS_START | AgingBucketsLinesDaysStart | ✅ |
| DAYS_TO | AgingBucketsLinesDaysTo | ✅ |
| LAST_UPDATE_DATE | AgingBucketsLinesLastUpdateDate | ✅ |
| LAST_UPDATE_LOGIN | AgingBucketsLinesLastUpdateLogin | — |
| LAST_UPDATED_BY | AgingBucketsLinesLastUpdatedBy | ✅ |
| OBJECT_VERSION_NUMBER | AgingBucketsLinesObjectVersionNumber | — |
| REPORT_HEADING1 | AgingBucketsLinesReportHeading1 | — |
| REPORT_HEADING2 | AgingBucketsLinesReportHeading2 | — |
| SET_ID | AgingBucketsLinesSetId | — |
| TYPE | AgingBucketsLinesType | — |

### [[agingbucketlinesextractpvo|AgingBucketLinesExtractPVO]] (OTHER · BICC: 15/31)

| Coluna da Tabela | Atributo do PVO | BICC |
|------------------|-----------------|------|
| AGING_BUCKET_ID | ArAgingBucketLinesBAgingBucketId | ✅ |
| AGING_BUCKET_LINE_ID | ArAgingBucketLinesBAgingBucketLineId | ✅ |
| ATTRIBUTE1 | ArAgingBucketLinesBAttribute1 | — |
| ATTRIBUTE10 | ArAgingBucketLinesBAttribute10 | — |
| ATTRIBUTE11 | ArAgingBucketLinesBAttribute11 | — |
| ATTRIBUTE12 | ArAgingBucketLinesBAttribute12 | — |
| ATTRIBUTE13 | ArAgingBucketLinesBAttribute13 | — |
| ATTRIBUTE14 | ArAgingBucketLinesBAttribute14 | — |
| ATTRIBUTE15 | ArAgingBucketLinesBAttribute15 | — |
| ATTRIBUTE2 | ArAgingBucketLinesBAttribute2 | — |
| ATTRIBUTE3 | ArAgingBucketLinesBAttribute3 | — |
| ATTRIBUTE4 | ArAgingBucketLinesBAttribute4 | — |
| ATTRIBUTE5 | ArAgingBucketLinesBAttribute5 | — |
| ATTRIBUTE6 | ArAgingBucketLinesBAttribute6 | — |
| ATTRIBUTE7 | ArAgingBucketLinesBAttribute7 | — |
| ATTRIBUTE8 | ArAgingBucketLinesBAttribute8 | — |
| ATTRIBUTE9 | ArAgingBucketLinesBAttribute9 | — |
| ATTRIBUTE_CATEGORY | ArAgingBucketLinesBAttributeCategory | — |
| BUCKET_SEQUENCE_NUM | ArAgingBucketLinesBBucketSequenceNum | ✅ |
| CREATED_BY | ArAgingBucketLinesBCreatedBy | ✅ |
| CREATION_DATE | ArAgingBucketLinesBCreationDate | ✅ |
| DAYS_START | ArAgingBucketLinesBDaysStart | ✅ |
| DAYS_TO | ArAgingBucketLinesBDaysTo | ✅ |
| LAST_UPDATE_DATE | ArAgingBucketLinesBLastUpdateDate | ✅ |
| LAST_UPDATE_LOGIN | ArAgingBucketLinesBLastUpdateLogin | ✅ |
| LAST_UPDATED_BY | ArAgingBucketLinesBLastUpdatedBy | ✅ |
| OBJECT_VERSION_NUMBER | ArAgingBucketLinesBObjectVersionNumber | ✅ |
| REPORT_HEADING1 | ArAgingBucketLinesBReportHeading1 | ✅ |
| REPORT_HEADING2 | ArAgingBucketLinesBReportHeading2 | ✅ |
| SET_ID | ArAgingBucketLinesBSetId | ✅ |
| TYPE | ArAgingBucketLinesBType | ✅ |

---

## 📚 Referências

- Oracle Fusion Cloud Financials — Accounts Receivable Tables (OEDMF Release 13).
- Oracle BICC — AR Aging Subject Area Documentation.
- Oracle Fusion Cloud ERP Schema Reference (Release 25A).
