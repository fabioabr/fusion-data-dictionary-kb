...
id: DOC-HCM-395
doc_type: system-doc
title: "HWM_TM_REP_M_PJC_DOC_ATRBS_V — View de Atributos Documentos de Projeto (PJC) de Medidas Reportadas"
...
View que consolida os **atributos de documentos de projeto (pjc)** das medidas (measures) de entradas reportadas de tempo. Foco em atributos de documentos de projeto (Project Costing) associados às medidas reportadas.
...
- **Consulta de atributos documentos de projeto (pjc):** acesso consolidado a dados de documentos de projeto (pjc) nas medidas reportadas.
...
- [[per_all_people_f]] — via `PERSON_ID` (pessoa dos atributos de documento de projeto)
...
### Consultar atributos documentos de projeto (pjc) de medidas reportadas
```sql
SELECT v.PERSON_ID, v.ENTRY_DATE, v.MEASURE,
       v.ATTRIBUTE_1, v.ATTRIBUTE_2
FROM   HWM_TM_REP_M_PJC_DOC_ATRBS_V v
...
- Específica para atributos de documentos de projeto (pjc) nas medidas reportadas.
