...
id: DOC-HCM-401
doc_type: system-doc
title: "HWM_TM_REP_S_HXT_ATRBS_V — View de Atributos HXT (Time & Labor) de Sumario Reportado"
...
View que consolida os **atributos de hxt (time & labor)** no nivel de sumario das entradas reportadas de tempo. Foco em atributos de Time & Labor (HXT) no sumario reportado.
...
- **Consulta sumarizada:** atributos de hxt (time & labor) consolidados por sumario.
...
- [[per_all_people_f]] — via `PERSON_ID` (pessoa dos atributos de cartao de ponto)
...
### Consultar atributos de sumario hxt (time & labor)
```sql
SELECT v.PERSON_ID, v.ENTRY_DATE, v.SUMMARY_MEASURE,
       v.ATTRIBUTE_1, v.STATUS
FROM   HWM_TM_REP_S_HXT_ATRBS_V v
...
- Especifica para atributos de hxt (time & labor) no nivel de sumario.
