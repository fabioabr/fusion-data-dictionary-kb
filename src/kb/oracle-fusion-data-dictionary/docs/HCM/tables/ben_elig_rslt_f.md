...
id: DOC-HCM-043
doc_type: system-doc
title: "BEN_ELIG_RSLT_F — Resultados de Elegibilidade"
...
aliases:
  - BEN_ELIG_RSLT_F
  - ben_elig_rslt_f
  - resultados-elegibilidade-beneficios
  - eligibility-results
  - ben-elig-rslt
...
Armazena os **resultados de avaliação de elegibilidade** para cada participante por plano. Indica se o colaborador é elegível ou não para cada plano de benefício.
...
- **Resultado de avaliação:** Registra se o colaborador é elegível ou não.
- **Histórico:** Mantém histórico de avaliações de elegibilidade.
- **Workflow:** Base para o fluxo de inscrição em benefícios.
...
### Consulta de resultados de elegibilidade
```sql
SELECT * FROM BEN_ELIG_RSLT_F
WHERE  ROWNUM <= 100;
```
...
- Tabela do módulo Benefits (Resultados de Elegibilidade).
