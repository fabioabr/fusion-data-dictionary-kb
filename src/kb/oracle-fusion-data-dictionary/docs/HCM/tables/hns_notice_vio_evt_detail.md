...
id: DOC-HCM-120
doc_type: system-doc
title: "HNS_NOTICE_VIO_EVT_DETAIL — Detalhes de Eventos de Notificação/Violação"
...
aliases:
  ...
  - hns_notice_vio_evt_detail
  - hns-notice-vio-evt-detail
  - DOC-HCM-120
  - detalhes-de-eventos-de-notificação/violação
...
Armazena os **detalhes específicos de eventos de notificação e violação** no módulo de Saúde e Segurança do Trabalho. Complementa `HNS_INCIDENTS_DETAIL` com informações granulares do tipo de evento.
...
- **Registro detalhado:** Informações específicas de eventos de notificação e violação.
- **Classificação:** Categorização granular do evento de notificação e violação.
...
- **Relatórios:** Estatísticas específicas de eventos de notificação e violação.
...
- Tabela complementar a `HNS_INCIDENTS_DETAIL` com dados específicos de notificação e violação.
...
- Dados são relevantes para relatórios regulatórios específicos de notificação e violação.
