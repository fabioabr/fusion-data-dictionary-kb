...
id: DOC-HCM-192
doc_type: system-doc
title: "HRQ_QSTN_ANSWERS_TL — Traduções de Opções de Resposta"
...
aliases:
  - HRQ_QSTN_ANSWERS_TL
  - hrq_qstn_answers_tl
  - traduções-de-opções-de-resposta
...
Tabela de **traduções** de opções de resposta de questões (base). Padrão Oracle MLS.
...
|---|--------|------|-------|-----------|-----------|-----|-----------|
| 1 | ANSWER_ID | NUMBER(18) | NOT NULL | PK/FK | Identificador | [[hrq_qstn_answers_b]] | 🟢 95% |
...
- [[hrq_qstn_answers_b]] — via `ANSWER_ID` (registro base do cadastro)
...
FROM   HRQ_QSTN_ANSWERS_TL tl
WHERE  tl.ANSWER_ID = :p_id AND tl.LANGUAGE = USERENV('LANG');
```
...
- Chave primária composta: `ANSWER_ID` + `LANGUAGE`.
