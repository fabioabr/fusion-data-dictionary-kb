...
id: DOC-HCM-388
doc_type: system-doc
title: "HWM_TM_REP_ATRB_TS_VAL_V — View de Valores Timestamp de Atributos Reportados"
...
View que apresenta os **valores do tipo Timestamp** (TS) dos atributos de entradas reportadas de tempo. Facilita a consulta de atributos com valores timestamp sem necessidade de conversão.
...
- **Consulta de atributos timestamp:** acesso direto a valores timestamp de entradas reportadas.
- **Relatórios:** facilita extração de atributos timestamp sem conversão de tipo.
...
| 4 | TS_VALUE | TIMESTAMP | NULL | Dados | Valor timestamp do atributo | — | 🟡 60% |
...
- [[hwm_tm_rep_atrbs]] — via `REP_ATRB_ID` (atributo de reporte do valor timestamp)
...
### Consultar valores timestamp de atributos reportados
```sql
SELECT v.ATTRIBUTE_NAME, v.TS_VALUE, v.REP_ENTRY_ID
FROM   HWM_TM_REP_ATRB_TS_VAL_V v
...
- Retorna apenas atributos com valores do tipo timestamp.
...
- [Oracle Docs — HWM_TM_REP_ATRB_TS_VAL_V](https://docs.oracle.com/en/cloud/saas/human-resources/25a/oedmf/hwmtmrepatrbtsvalv.html)
...
### [[attributetimestampvaluepvo|AttributeTimestampValuePVO]] (HCM · BICC: 6/28)
...
| ATTRIBUTE_TIMESTAMP_VALUE | AttributeTimestampValue | ✅ |
