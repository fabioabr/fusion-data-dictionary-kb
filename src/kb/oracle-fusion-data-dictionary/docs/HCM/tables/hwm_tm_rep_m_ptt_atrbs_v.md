...
id: DOC-HCM-397
doc_type: system-doc
title: "HWM_TM_REP_M_PTT_ATRBS_V — View de Atributos Período de Tempo (PTT) de Medidas Reportadas"
...
View que consolida os **atributos de período de tempo (ptt)** das medidas (measures) de entradas reportadas de tempo. Foco em atributos de período de tempo (Period Time Type) associados às medidas reportadas.
...
- **Consulta de atributos período de tempo (ptt):** acesso consolidado a dados de período de tempo (ptt) nas medidas reportadas.
...
- [[per_all_people_f]] — via `PERSON_ID` (pessoa dos atributos de horas extras)
...
### Consultar atributos período de tempo (ptt) de medidas reportadas
```sql
SELECT v.PERSON_ID, v.ENTRY_DATE, v.MEASURE,
       v.ATTRIBUTE_1, v.ATTRIBUTE_2
FROM   HWM_TM_REP_M_PTT_ATRBS_V v
...
- Específica para atributos de período de tempo (ptt) nas medidas reportadas.
