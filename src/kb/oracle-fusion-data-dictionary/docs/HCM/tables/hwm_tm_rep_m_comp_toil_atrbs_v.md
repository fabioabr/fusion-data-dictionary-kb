...
id: DOC-HCM-393
doc_type: system-doc
title: "HWM_TM_REP_M_COMP_TOIL_ATRBS_V — View de Atributos Compensação/TOIL de Medidas Reportadas"
...
aliases:
  ...
  - hwm_tm_rep_m_comp_toil_atrbs_v
  - hwm-tm-rep-m-comp-toil-atrbs-v
...
View que consolida os **atributos de compensação/toil** das medidas (measures) de entradas reportadas de tempo. Foco em atributos de compensação e Time Off In Lieu (TOIL) associados às medidas reportadas.
...
- **Consulta de atributos compensação/toil:** acesso consolidado a dados de compensação/toil nas medidas reportadas.
...
- [[per_all_people_f]] — via `PERSON_ID` (pessoa dos atributos de compensacao)
...
### Consultar atributos compensação/toil de medidas reportadas
```sql
SELECT v.PERSON_ID, v.ENTRY_DATE, v.MEASURE,
       v.ATTRIBUTE_1, v.ATTRIBUTE_2
FROM   HWM_TM_REP_M_COMP_TOIL_ATRBS_V v
...
- Específica para atributos de compensação/toil nas medidas reportadas.
