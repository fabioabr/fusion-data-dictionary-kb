...
id: DOC-HCM-399
doc_type: system-doc
title: "HWM_TM_REP_S_ABS_TEP_ATRBS_V — View de Atributos Ausência por Tipo de Entrada (TEP) de Sumário Reportado"
...
View que consolida os **atributos de ausência por tipo de entrada (tep)** no nível de sumário das entradas reportadas de tempo. Foco em atributos de ausência por tipo de entrada de período (Time Entry Period) no sumário.
...
- **Consulta sumarizada:** atributos de ausência por tipo de entrada (tep) consolidados por sumário.
...
- [[per_all_people_f]] — via `PERSON_ID` (pessoa dos atributos de ausencia temporaria)
...
### Consultar atributos de sumário ausência por tipo de entrada (tep)
```sql
SELECT v.PERSON_ID, v.ENTRY_DATE, v.SUMMARY_MEASURE,
       v.ATTRIBUTE_1, v.STATUS
FROM   HWM_TM_REP_S_ABS_TEP_ATRBS_V v
...
- Específica para atributos de ausência por tipo de entrada (tep) no nível de sumário.
