...
id: DOC-HCM-398
doc_type: system-doc
title: "HWM_TM_REP_S_ABS_HP_ATRBS_V — View de Atributos Ausência por Plano (HP) de Sumário Reportado"
...
View que consolida os **atributos de ausência por plano (hp)** no nível de sumário das entradas reportadas de tempo. Foco em atributos de ausência por plano de horas (Hours Plan) no sumário.
...
- **Consulta sumarizada:** atributos de ausência por plano (hp) consolidados por sumário.
...
- [[per_all_people_f]] — via `PERSON_ID` (pessoa dos atributos de ausencia por hora)
...
### Consultar atributos de sumário ausência por plano (hp)
```sql
SELECT v.PERSON_ID, v.ENTRY_DATE, v.SUMMARY_MEASURE,
       v.ATTRIBUTE_1, v.STATUS
FROM   HWM_TM_REP_S_ABS_HP_ATRBS_V v
...
- Específica para atributos de ausência por plano (hp) no nível de sumário.
