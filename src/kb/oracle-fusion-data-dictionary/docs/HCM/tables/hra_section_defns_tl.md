...
id: DOC-HCM-114
doc_type: system-doc
title: "HRA_SECTION_DEFNS_TL — Definições de Seções de Avaliação (Tradução)"
...
aliases:
  ...
  - hra_section_defns_tl
  - hra-section-defns-tl
  - DOC-HCM-114
  - definições-de-seções-de-avaliação-(tradução)
...
Armazena as **traduções** dos nomes e descrições das seções de avaliação. Tabela _TL que complementa `HRA_SECTION_DEFNS_B`.
...
- **Multilinguismo:** Nomes de seções em múltiplos idiomas.
...
|---|--------|------|-------|-----------|-----------|-----|-----------|
| 1 | SECTION_DEFN_ID | NUMBER(18) | NOT NULL | PK/FK | Identificador da seção | [[hra_section_defns_b]] | 🟢 90% |
| 2 | LANGUAGE | VARCHAR2(4) | NOT NULL | PK | Código do idioma | — | 🟢 95% |
| 3 | SOURCE_LANG | VARCHAR2(4) | NOT NULL | Controle | Idioma de origem | — | 🟢 95% |
| 4 | NAME | VARCHAR2(240) | NOT NULL | Tradução | Nome da seção traduzido | — | 🟡 80% |
...
- [[hra_section_defns_b]] — via `SECTION_DEFN_ID` (registro base do cadastro)
...
FROM   HRA_SECTION_DEFNS_TL tl
WHERE  tl.SECTION_DEFN_ID = :p_id
  AND  tl.LANGUAGE = 'PTBR';
```

### Com tradução
```sql
SELECT b.SECTION_CODE, tl.NAME
FROM   HRA_SECTION_DEFNS_B b
JOIN   HRA_SECTION_DEFNS_TL tl ON b.SECTION_DEFN_ID = tl.SECTION_DEFN_ID
...
- Chave primária composta: `SECTION_DEFN_ID` + `LANGUAGE`.
