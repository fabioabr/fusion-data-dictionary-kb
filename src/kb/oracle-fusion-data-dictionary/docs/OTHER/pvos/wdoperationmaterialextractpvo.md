---
id: DOC-OTHER-PVO-WDOperationMaterialExtractPVO
doc_type: system-doc
title: "WDOperationMaterialExtractPVO — PVO Cross-Module"
system: Oracle Fusion Cloud ERP
module: Cross-Module
domain: Técnico
owner: fabio.patria
team: dados
status: draft
confidentiality: internal
tags:
  - oracle-fusion
  - other
  - data-dictionary
  - pvo
  - bicc
aliases:
  - WDOperationMaterialExtractPVO
  - wdoperationmaterialextractpvo
source_format: markdown
conversion_pipeline: json-to-md-v1
conversion_quality: 100
qa_score: 0
qa_status: not_reviewed
created_at: 2026-03-31
updated_at: 2026-03-31
---

# WDOperationMaterialExtractPVO

## 📌 Visão Geral

View Object para extração BICC de dados de WDOperation Material Extract. Acessa as tabelas: WIS_WD_OPERATION_MATERIALS.

**Caminho:** `FscmTopModelAM.ScmExtractAM.WisBiccExtractAM.WDOperationMaterialExtractPVO`

| Atributos | Tabelas | PKs | BICC | BICC % |
|-----------|---------|-----|------|--------|
| 67 | 1 | 1 | 67 | 100% |

---

## 🔗 Tabelas Relacionadas

- [[wis_wd_operation_materials|WIS_WD_OPERATION_MATERIALS]] — 67 atributos (1 PKs, 67 BICC)

---

## ⚙️ Atributos

### [[wis_wd_operation_materials|WIS_WD_OPERATION_MATERIALS]]

| # | Atributo | Coluna | PK | BICC |
|---|----------|--------|----|------|
| 1 | AdhocFlag | ADHOC_FLAG | — | ✅ |
| 2 | AttributeCategory | ATTRIBUTE_CATEGORY | — | ✅ |
| 3 | AttributeChar1 | ATTRIBUTE_CHAR1 | — | ✅ |
| 4 | AttributeChar10 | ATTRIBUTE_CHAR10 | — | ✅ |
| 5 | AttributeChar11 | ATTRIBUTE_CHAR11 | — | ✅ |
| 6 | AttributeChar12 | ATTRIBUTE_CHAR12 | — | ✅ |
| 7 | AttributeChar13 | ATTRIBUTE_CHAR13 | — | ✅ |
| 8 | AttributeChar14 | ATTRIBUTE_CHAR14 | — | ✅ |
| 9 | AttributeChar15 | ATTRIBUTE_CHAR15 | — | ✅ |
| 10 | AttributeChar16 | ATTRIBUTE_CHAR16 | — | ✅ |
| 11 | AttributeChar17 | ATTRIBUTE_CHAR17 | — | ✅ |
| 12 | AttributeChar18 | ATTRIBUTE_CHAR18 | — | ✅ |
| 13 | AttributeChar19 | ATTRIBUTE_CHAR19 | — | ✅ |
| 14 | AttributeChar2 | ATTRIBUTE_CHAR2 | — | ✅ |
| 15 | AttributeChar20 | ATTRIBUTE_CHAR20 | — | ✅ |
| 16 | AttributeChar3 | ATTRIBUTE_CHAR3 | — | ✅ |
| 17 | AttributeChar4 | ATTRIBUTE_CHAR4 | — | ✅ |
| 18 | AttributeChar5 | ATTRIBUTE_CHAR5 | — | ✅ |
| 19 | AttributeChar6 | ATTRIBUTE_CHAR6 | — | ✅ |
| 20 | AttributeChar7 | ATTRIBUTE_CHAR7 | — | ✅ |
| 21 | AttributeChar8 | ATTRIBUTE_CHAR8 | — | ✅ |
| 22 | AttributeChar9 | ATTRIBUTE_CHAR9 | — | ✅ |
| 23 | AttributeDate1 | ATTRIBUTE_DATE1 | — | ✅ |
| 24 | AttributeDate2 | ATTRIBUTE_DATE2 | — | ✅ |
| 25 | AttributeDate3 | ATTRIBUTE_DATE3 | — | ✅ |
| 26 | AttributeDate4 | ATTRIBUTE_DATE4 | — | ✅ |
| 27 | AttributeDate5 | ATTRIBUTE_DATE5 | — | ✅ |
| 28 | AttributeNumber1 | ATTRIBUTE_NUMBER1 | — | ✅ |
| 29 | AttributeNumber10 | ATTRIBUTE_NUMBER10 | — | ✅ |
| 30 | AttributeNumber2 | ATTRIBUTE_NUMBER2 | — | ✅ |
| 31 | AttributeNumber3 | ATTRIBUTE_NUMBER3 | — | ✅ |
| 32 | AttributeNumber4 | ATTRIBUTE_NUMBER4 | — | ✅ |
| 33 | AttributeNumber5 | ATTRIBUTE_NUMBER5 | — | ✅ |
| 34 | AttributeNumber6 | ATTRIBUTE_NUMBER6 | — | ✅ |
| 35 | AttributeNumber7 | ATTRIBUTE_NUMBER7 | — | ✅ |
| 36 | AttributeNumber8 | ATTRIBUTE_NUMBER8 | — | ✅ |
| 37 | AttributeNumber9 | ATTRIBUTE_NUMBER9 | — | ✅ |
| 38 | AttributeTimestamp1 | ATTRIBUTE_TIMESTAMP1 | — | ✅ |
| 39 | AttributeTimestamp2 | ATTRIBUTE_TIMESTAMP2 | — | ✅ |
| 40 | AttributeTimestamp3 | ATTRIBUTE_TIMESTAMP3 | — | ✅ |
| 41 | AttributeTimestamp4 | ATTRIBUTE_TIMESTAMP4 | — | ✅ |
| 42 | AttributeTimestamp5 | ATTRIBUTE_TIMESTAMP5 | — | ✅ |
| 43 | BasisType | BASIS_TYPE | — | ✅ |
| 44 | ComponentSequenceId | COMPONENT_SEQUENCE_ID | — | ✅ |
| 45 | ComponentSequenceIdPath | COMPONENT_SEQUENCE_ID_PATH | — | ✅ |
| 46 | CreatedBy | CREATED_BY | — | ✅ |
| 47 | CreationDate | CREATION_DATE | — | ✅ |
| 48 | InventoryItemId | INVENTORY_ITEM_ID | — | ✅ |
| 49 | InverseMaterialQuantity | INVERSE_MATERIAL_QUANTITY | — | ✅ |
| 50 | JobDefinitionName | JOB_DEFINITION_NAME | — | ✅ |
| 51 | JobDefinitionPackage | JOB_DEFINITION_PACKAGE | — | ✅ |
| 52 | LastUpdateDate | LAST_UPDATE_DATE | — | ✅ |
| 53 | LastUpdateLogin | LAST_UPDATE_LOGIN | — | ✅ |
| 54 | LastUpdatedBy | LAST_UPDATED_BY | — | ✅ |
| 55 | MaterialQuantity | MATERIAL_QUANTITY | — | ✅ |
| 56 | MaterialSeqNumber | MATERIAL_SEQ_NUMBER | — | ✅ |
| 57 | MaterialType | MATERIAL_TYPE | — | ✅ |
| 58 | ObjectVersionNumber | OBJECT_VERSION_NUMBER | — | ✅ |
| 59 | OrganizationId | ORGANIZATION_ID | — | ✅ |
| 60 | RequestId | REQUEST_ID | — | ✅ |
| 61 | SupplyLocatorId | SUPPLY_LOCATOR_ID | — | ✅ |
| 62 | SupplySubinventoryName | SUPPLY_SUBINVENTORY_NAME | — | ✅ |
| 63 | SupplyType | SUPPLY_TYPE | — | ✅ |
| 64 | UomCode | UOM_CODE | — | ✅ |
| 65 | WdOperationId | WD_OPERATION_ID | — | ✅ |
| 66 | WdOperationMaterialId | WD_OPERATION_MATERIAL_ID | 🔑 | ✅ |
| 67 | YieldFactor | YIELD_FACTOR | — | ✅ |

---

## 📚 Referências

- [[other-module-data-dictionary]] — Dossiê do módulo OTHER
